...
It could be used with a source connector to pull raw data from external sources, generate embeddings, and write them to a Kafka topic. Alternatively, a sink connector can read pre-computed embeddings from a Kafka topic (where embeddings were generated by other components like Kafka Streams or Flink) and write them to the vector database.
